...
- **nikctl reads its flags wherever they are written.** Go's flag package stops parsing at the first non-flag argument, so `nikctl secrets write openai_key --home /nik` silently dropped --home and `nikctl config set a.b c --home /nik` counted four positionals and exited on a usage message — both from a line an operator wrote the only way anyone writes one they are still editing. `parseFlags` in `cmd/nikctl` hoists the flags a command actually defines to the front and leaves everything else in order behind a `--`, so a trailing flag works and `nikctl shell ls -la` still passes -la to the shell. Any new nikctl command parses through it, never `flagSet.Parse` directly, and takes exactly the arguments it names — a leftover positional is what a misspelled flag turns into once parsing is done, so counting it and carrying on is how `secrets write openai_key --hom /nik` wrote to the wrong home in silence. Applied 2026-08-22.
