...
func TestBuildConversationInputIncludesSessionBlock(t *testing.T) {
	...
	lines := BuildConversationInput("conv-1", conv, msgs, map[string]string{"msg-1": "Alice"}, SessionContext{
		Lines: []string{
			"Platform: whatsapp",
			"Type: dm",
			"Contact: Alice",
...
func TestBuildConversationInputNoCanonicalRefs(t *testing.T) {
	...
	lines := BuildConversationInput("conv-1", conv, msgs, labels, SessionContext{}, nil)
	out := strings.Join(lines, "\n")
