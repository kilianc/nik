...
- **If secrets access fails, stop immediately.** Do not work around missing credentials -- no fallback APIs, no scraping alternatives, no "skip secrets for now." Report `failed` with the exact error from `./secrets/cli` so your manager can escalate.
...
- If you need credentials or config, check the workspace, use the secrets skill, check environment variables.
