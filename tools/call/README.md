...
LLM tools (`describe_media`), contact tools, db query tools (`db_query`), shell tools (`shell`), skill tools (`load_skill`), and task tools (`task_spawn`, `task_retry`, `task_list`, `task_status`, `task_cancel`). Live platform actions (`message_send`, `message_react`, typing/presence) are excluded.
