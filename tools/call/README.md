...
LLM tools (`describe_media`), canonical DB-backed messaging tools (`message_update_media_description`), contact/search tools, shell tools (`shell`), web search (`web_search`), and skill tools (`load_skill`). Live platform actions (`message_reply`, `message_react`, typing/presence) are excluded.
